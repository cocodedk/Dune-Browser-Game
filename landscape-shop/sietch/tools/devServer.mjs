// landscape-shop/sietch/tools/devServer.mjs
...
// devServer.mjs (never imported — shops don't share code,
// landscape-shop/docs/gauntlet-loop.md) with this shop's own path and
// port so every shop harness can run at once without colliding.
...
export async function ensureDevServer(port) {
  ...
  const server = spawn('npx', ['vite', 'landscape-shop/sietch', '--port', String(port), '--strictPort'], {
  ...
  const ready = await new Promise((resolve) => {
    ...
    const watch = (chunk) => {
      // Strip ANSI escapes first: vite 6's "Local:" line bolds the port
      // digits on their own (colon, then ESC[1m, then the digits), so a
      // plain `:${port}` substring check never sees them adjacent and the
      // bring-up silently timed out at 60s until this was found.
      const clean = String(chunk).replace(ANSI_ESCAPE, '')
