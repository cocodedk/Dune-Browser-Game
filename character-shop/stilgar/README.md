# Stilgar — character shop

Scaffolded by `npm run cast:new -- stilgar`. Build and gauntlet-test
stilgar here, separately from the game and from every other shop, in
...
```bash
npm run cast:stilgar          # dev server for the test area
npm run cast:stilgar:check    # type-check (tsc -p character-shop/stilgar)
npm run cast:stilgar:build    # production build of the shop alone
```
...
npm run cast:stilgar:check
npx vitest run character-shop/stilgar
