# Chani — character shop

Scaffolded by `npm run cast:new -- chani`. Build and gauntlet-test
chani here, separately from the game and from every other shop, in
...
```bash
npm run cast:chani          # dev server for the test area
npm run cast:chani:check    # type-check (tsc -p character-shop/chani)
npm run cast:chani:build    # production build of the shop alone
```
...
npm run cast:chani:check
npx vitest run character-shop/chani
