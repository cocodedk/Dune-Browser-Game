...
import { bakeFace, segmentDistance, type RGB } from './faceBaker'
 ...
 */
export function movingMapTexture(): DataTexture {
  return bakeFace(SIZE, (nx, ny) => {
    const border = edge(nx, ny)
    if (border) return border
    // Heading band and its lubber line.
    if (ny > 0.76) {
      if (Math.abs(nx) < 0.035) return OWNSHIP
      if (Math.abs(((nx * 100 + 200) % 16) - 8) < 1.6) return TICK
      return BAND
    }
    const shipY = -0.34
    for (const [ax, ay, bx, by] of LEGS) {
      if (segmentDistance(nx, ny, ax, ay, bx, by) < 0.035) return ROUTE
    }
    for (const [wx, wy] of WAYPOINTS) {
      if (diamond(nx, ny, wx, wy, 0.09) && !diamond(nx, ny, wx, wy, 0.05)) return WAYPOINT
    }
    // Ownship: a filled arrowhead pointing up the page.
    const up = ny - shipY
    ...
    const field = Math.sin(nx * 4.2 + ny * 1.4) + Math.cos(ny * 3.6 - nx * 0.9)
    if (Math.abs(((field * 1.25 + 30) % 1) - 0.5) < 0.07) return CONTOUR
    if (Math.abs(((nx * 100 + 200) % 25) - 12.5) < 1) return GRID
    if (Math.abs(((ny * 100 + 200) % 25) - 12.5) < 1) return GRID
    return VOID
  })
}

/** Bar rows: centre y, fill fraction 0..1, red limit fraction, colour. */
const BARS: ReadonlyArray<readonly [number, number, number, RGB]> = [
  [0.46, 0.82, 0.9, GREEN],
  [0.16, 0.64, 0.88, GREEN],
  [-0.14, 0.91, 0.86, AMBER],
 ...
 */
export function systemsPageTexture(): DataTexture {
  return bakeFace(SIZE, (nx, ny) => {
    const border = edge(nx, ny)
    if (border) return border
    if (ny > 0.76) {
    ...
    for (const [cy, fill, limit, colour] of BARS) {
