...
| `src/game-engine/economy/actRun.ts:25` — `actView()` hard-codes `palaceHeld: true`, so `transitions.ts:54` (`if (!view.palaceHeld) return 'loss_palace'`) can never fire: the `loss_palace` ending is **decorative** today. (Added in round 3 after the WP00 evidence audit; baseline proof in `captures/captures.md` "Endings coverage".) | Victory/ending authority: an authored ending route exists in the act machine but is dead at the view layer — a live hit on the risk register's "One ending is decorative" row. | WP01 |
...
| **Dialogue effects are a fourth independent `world.player.spice` writer.** `src/game-engine/DialogueSystem.ts:84-86` — `if (effect.spiceDelta) { world.player.spice = Math.max(0, world.player.spice + effect.spiceDelta) }`; five authored positive `spiceDelta` effects exist in the dialogue data. (Added in round 3 after the WP00 evidence audit: the function's other two effect writes were already inventoried in category 4, but this spice write had no owner.) | Dialogue-effect spice credit vs. the three day-boundary paths in the row above — a fourth writer to the same field, outside any structured command. | WP02 |
...
| 3. PoC victory goals gating/ending a campaign | 8 (7 counted + 1 debug-only note excluded, see "Out of campaign scope"; the `actRun.ts:25` decorative-`loss_palace` row was added in round 3) | No — includes a direct write to `world.ending`, the act machine's reserved field |
...
| 6. Duplicate resource paths | 4 confirmed duplicates (the `DialogueSystem.ts:84-86` spice writer was added in round 3) + 1 explicitly ruled-out check + 1 parallel-simulation note (owner deviates to WP04) | No |
